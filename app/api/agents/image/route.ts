...
import { IMAGE_PROMPT_SYSTEM_PROMPT } from "@/lib/agents/system-prompts";
...
export async function POST(request: NextRequest) {
  ...
  const { concept, style, model, apiKey } = await request.json();
  ...
  const userMessage = style
    ? `Generate image prompts for: "${concept}"\n\nPreferred style: ${style}`
    : `Generate image prompts for: "${concept}"`;
    ...
    if (provider === "anthropic") {
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${modelId}:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            system_instruction: { parts: [{ text: IMAGE_PROMPT_SYSTEM_PROMPT }] },
