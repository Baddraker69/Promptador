...
import { PROMPT_OPTIMIZER_SYSTEM_PROMPT } from "@/lib/agents/system-prompts";
  ...
  try {
    ...
    if (provider === "anthropic") {
      ...
      const message = await client.messages.create({
        ...
        messages: [{ role: "user", content: `Optimise this prompt:\n\n${prompt}` }],
      });
      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: "user", content: `Optimise this prompt:\n\n${prompt}` },
          ],
          max_tokens: 4096,
        }),
      });
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${modelId}:generateContent?key=${apiKey}`,
        {
          ...
          body: JSON.stringify({
            system_instruction: { parts: [{ text: PROMPT_OPTIMIZER_SYSTEM_PROMPT }] },
            contents: [{ parts: [{ text: `Optimise this prompt:\n\n${prompt}` }] }],
          }),
        }
      );
      const data = await response.json();
